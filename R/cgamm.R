cgamm = function(formula, nsim = 0, family = gaussian(), cpar = 1.2, data = NULL,
  ...
  id <- mf[,nc]
  szs <- unname(table(id))
  #new: used in plot.cgamm to compute vcov
  df_vars <- mf[, 2:(nc-1), drop=FALSE]
  nms_df <- NULL
  for (i in 2:(nc-1)) {
    if (is.numeric(attributes(mf[,i])$shape)) {
      ...
      xid <- xid + 1
      #new
      nms_df <- c(nms_df, attributes(mf[,i])$nm)
    }
  ...
  rslt <- cgamm.fit(y = y, xmat = xmat0, zmat = zmat, id = id, shapes = shapes0,
                    ...
                    reml = reml, multicore = multicore, CIC = CIC, nAGQ = nAGQ)
  rslt$id <- id
  rslt$szs <- szs
  rslt$shapes <- shapes0
  rslt$space <- sps0
  ...
  rslt$call <- cl
  colnames(df_vars) <- nms_df
  rslt$df_vars <- df_vars
...
cgamm.fit = function(y, xmat, zmat, id, shapes, numknots, knots, space, nsim,
                     ...
                     bisect = FALSE, modlmer = FALSE, reml = TRUE, multicore = FALSE,
                     CIC = FALSE, nAGQ = 1L)  {
  ...
  if (capl > 0) {
    ...
    if (sum(shapes > 2 & shapes < 5 | shapes > 10 & shapes < 13) > 0 & capk > 0) {
      xvec = xmat[, shapes > 2 & shapes < 5 | shapes > 10 & shapes < 13, drop = FALSE]
      bigmat = rbind(1:n*0 + 1, t(zmat), t(xvec), delta)
      np = 1 + capk + sum(shapes > 2 & shapes < 5 | shapes > 10 & shapes < 13)  + capms
    } else if (sum(shapes > 2 & shapes < 5 | shapes > 10 & shapes < 13) > 0 & capk == 0) {
      xvec = xmat[, shapes > 2 & shapes < 5 | shapes > 10 & shapes < 13, drop = FALSE]
      bigmat = rbind(1:n*0 + 1, t(xvec), delta)
  ...
  if (!wt.iter) {
    ...
    while (diff > 1e-7 & nrep < 10) {
      ...
      if (!balanced) {
        ...
        if (modlmer) {
          ...
          if (reml) {
            #ansi = try(ansi0<-uniroot(fth2rm, c(1e-10, 1e+3), 
            #                          szs=szs, ycl=ecl, N=n, xcl=xms, p=edf, type='ub', xtx=xtx, xtx2=xtx2, 
            #                          xmat_face=dd, ones=ones), silent=TRUE)
            
            ansi <- try(ansi0<-uniroot(f = function(log_pen,...) fth2rm(exp(log_pen),...),
                                       interval = c(-8, 8), szs=szs, ycl=ecl, N=n, xcl=xms, 
                                       p=edf, type='ub', xtx=xtx, xtx2=xtx2, xmat_face=dd, ones=ones,
                                       extendInt = "yes", tol = .Machine$double.eps^0.25), silent=TRUE)
            ...
            ansi <- try(ansi0<-uniroot(f = function(log_pen,...) fth2rm(exp(log_pen),...),
                                       interval = c(-8, 8), szs=szs, ycl=ecl, N=n, 
                                       extendInt = "yes", tol = .Machine$double.eps^0.25), silent=TRUE)
          }
        ...
        if (modlmer) {
          ...
          if (reml) {
            ...
            #print (class(ansi))
            #print (ansi)
            #ansi = try(ansi0<-uniroot(fth2rm, c(1e-10, 1e+3), szs=szs, ycl=ecl, N=n, xcl=xms, p=edf,  type='b',
            #                          xtx=xtx, xtx2=xtx2, xmat_face=dd, ones=ones), silent=TRUE)
            ansi <- try(ansi0<-uniroot(f = function(log_pen,...) fth2rm(exp(log_pen),...),
                                       interval = c(-8, 8), szs=szs, ycl=ecl, N=n, xcl=xms, p=edf,
                                       type='b', xtx=xtx, xtx2=xtx2, xmat_face=dd, ones=ones, 
                                       extendInt = "yes", tol = .Machine$double.eps^0.25), silent=TRUE)
          } else {
            #ansi = try(ansi0<-uniroot(fth2, c(1e-10, 1e+3), szs=szs, ycl=ecl, N=n), silent=TRUE)
            ansi <- try(ansi0<-uniroot(f = function(log_pen,...) fth2rm(exp(log_pen),...),
    ...
    #siga = 1e-4
    #test!
    m0 = glmer(y ~ 0 + offset(etahat) + (1|id), family = binomial(link = "logit"), nAGQ = nAGQ)
    siga = as.numeric(attr(VarCorr(m0)$id, "stddev"))
    siga2hat = siga^2
    ahat = ranef(m0)$id[,1]
    diff = 10
    nrep = 0
    ...
    if(nAGQ > 1L){
      ...
      while (diff > 1e-7 & mdiff & nrep < 20) {
        ...
        #siga = 2^.5 #test code
        siga2hat = siga^2
        if(round(siga2hat, 8) == 0){
          break
        }
        #test!
        #ahat = favec2(ycl, sig=siga, avec=ahat, ecl=ecl) #almost the same as what glmer returns, depends on initializtion
        ahat = ranef(m0)$id[,1]
        ######
        ...
        if (nrep > 1) {
          ans <- suppressMessages(try(coneB(zvec, dsend, vmat = zsend, face = face), silent = TRUE))
        } else {
          ans <- suppressMessages(try(coneB(zvec, dsend, vmat = zsend), silent = TRUE))
          #face = ans$face
        }
    ...
    mod.lmer = suppressMessages(lmer(evec ~ -1+(1|id), REML=reml, 
  ...
  if(!wt.iter){
    ...
    uinv_mat = Matrix::bdiag(uinv_lst)
    uinv_mat = as.matrix(uinv_mat) / sig2hat^(.5)
      ...
      for (i in 1:capl) {
        ...
        if(family$family == "gaussian"){
          ansi = cgam.pv(y, xmat, zmat, shapes, delta=delta, np=np, capms=capms, numknotsuse=numknotsuse, varlist=varlist,
                         family=family, weights=uinv_mat, test_id=i, nsims=1000, skip=TRUE)
        } else {
          #if(nAGQ == 1L) {
          ansi = cgam.pv(zvec_all, xmat, zmat, shapes, delta=delta, np=np, capms=capms, numknotsuse=numknotsuse, varlist=varlist,
                         family=gaussian(link='identity'), weights=uinvkeep, test_id=i, nsims=1000, skip=TRUE)
          #cat('finished pval')
          #}
          # if(nAGQ > 1L) {
          ...
          #                     test_id=i, nsims=nsim, nAGQ=nAGQ, uinv_vm = uinvkeep)
          # }
        }
        pvi = ansi$pv
        #print (pvi)
  ...
  if (capk > 0) {
    ...
    if(family$family == "gaussian"){
      ...
      zvec_all = etahatkeep_all + (y - muhatkeep_all) / family$variance(muhatkeep_all)
      dd = t(bigmat)
      sse1 = sum((uinvkeep %*% zvec_all - uinvkeep %*% dd %*% coefskeep)^2)
      #cgam.pvz should include r.f. in fitting
      #print (sse1)
      #new: 2026: should use cgam.pvz directly?
      ansi = cgam.pvz(y=zvec_all, bigmat=bigmat, df_obs=df_obs, sse1=sse1, np=np, zid=zid, zid1=zid1, zid2=zid2,
                      muhat=NULL, etahat=NULL, coefskeep=coefskeep, wt.iter=FALSE,
                      family=gaussian(link='identity'), weights=prior.w, uinv_mat=uinvkeep)
      ...
      #                 ycl=ycl, siga=siga, ahat=ahat, ncl=ncl, szs=szs, nAGQ=nAGQ)
      #cat('finished pvz')
    }
    pvsz = ansi$pvs
    #print (pvsz)
  ...
  if(CIC){
    ...
    if(multicore){
      if(family$family == "gaussian"){
        edfs = parallel::mclapply(1:nsim,
                                  .compute_edf0_cgamm, bigmat_t, np, szs, uinv0, siga2hat, sig2hat,
                                  balanced, family,
                                  mc.cores = (cores))
      } else {
        edfs = parallel::mclapply(1:nsim,
                                  .compute_edf0_cgamm, bigmat_t, np, szs, uinv0=NULL, siga2hat, sig2hat=NULL,
                                  balanced, oneMat, family,
                                  mc.cores = (cores))
      }
    } else {
      if(family$family == "gaussian"){
    ...
    cpar = 1.2
    if(!wt.iter){
      negloglike = fmin(thhat, ncl, ecl, n, reml=FALSE)
    } else {
      #negloglike = fmin.witer(siga, ycl, ecl, ahat, nodes, nwt) #ignored the first term with only siga
      #almost the same as -logLik(m0); fmin.witer should be correct
      #test!
      if(round(siga, 8) == 0){
        negloglike = -logLik(m0)
      } else {
        negloglike = fmin.witer(siga, ycl, ecl, ahat, nodes, nwt) 
      }
    }
    llh = 2/n * as.numeric(negloglike)
    if ((n - np - cpar * (dfmean - np)) <= 0) {
      cic = llh + log(1 + 2 * dfmean / (dfmean - np))
      #cic = llh + 2 * dfmean / (dfmean - np)
    } else {
      if (n <= 200){
        cpar = 1.5
      }
      #cic = llh + 2 * dfmean / n
      cic = llh + log(1 + 2 * dfmean / (n - np - cpar * (dfmean - np)))
    }
  }
  #print (dim(coefskeep))
  if(!wt.iter){
    ...
    rslt = list(etahat = muhat, muhat = muhat, coefs = coefskeep, coefs_rf = coefs, bh = bh, zcoefs = zcoefs,
                ...
                muhatkeep = muhatkeep, uinv_mat = uinv_mat, cic = cic, edf0 = dfmean, varlist = varlist)
    ...
    rslt = list(etahat = etahat,  etahat_rf = etahatkeep_all,
                ...
                oneMat = oneMat, vm = vm, shat = shat, uinvkeep = uinvkeep, cic = cic, edf0 = dfmean, varlist = varlist)
  }
  return (rslt)
}

#####################################
#simulate edf0, from classo code.   #
#added in 2025 
#####################################
.compute_edf0_cgamm <- function(isim, bigmat_t, np, szs, 
                                uinv0=NULL, siga2hat, sig2hat=NULL, 
                                balanced=TRUE, oneMat=NULL,
                                family=gaussian(link = 'identity'))
{
  ...
  if (!wt.iter) {
    ...
    for (icl in 1:ncl) {
      ...
      ysi <- uinv %*% (rnorm(1, sd = siga2hat^.5) + rnorm(sz, sd = sig2hat^.5))
      #ysi <- uinv %*% (rep(rnorm(1, sd = siga2hat^.5), sz))
      #ysi <- uinv %*% rnorm(sz)
      ysim <- c(ysim, ysi)
      #ddi <- dd[st:ed, ,drop=FALSE]
      ddi <- bigmat_t[st:ed, ,drop=FALSE]
      ddtil <- rbind(ddtil, uinv %*% ddi)
      st <- ed + 1
    }
    dsend <- ddtil[, -c(1:np)]
    zsend <- ddtil[, 1:np]
    ...
    #nodes = out$nodes
    #nwt = out$weights
    #generate ahat from rnorm, then etahat and ysim, then muhat 
    cicfamily <- CicFamily(family)
    deriv.fun <- cicfamily$deriv.fun
    muhat.fun <- cicfamily$muhat.fun
    ncl <- length(szs)
    avec <- rnorm(ncl, sd = siga2hat^.5)
    etahat <- NULL
    muhat <- NULL
    ysim <- NULL
    for(icl in 1:ncl){
      sz <- szs[icl]
      etai <- rep(avec[icl], sz)
      etahat <- c(etahat, etai)
      mui <- 1 - 1/(1+exp(etai))
      muhat <- c(muhat, mui)
      yi <- rbinom(sz, size = 1, prob = mui)
      ysim <- c(ysim, yi)
    }
    nrep <- 0
    #n <- ncol(bigmat)
    ...
    while (diff > 1e-7 & mdiff & nrep < 20) {
      #print (diff)
      nrep <- nrep + 1
      #siga = nlminb(siga, fmin.witer, ycl=ycl, ecl=ecl, avec=ahat, nodes=nodes, n
      #              wt=nwt, lower = 1e-4, upper = 1000)$par
      #siga2hat = siga^2
      #ahat = favec2(ycl, sig=siga, avec=ahat, ecl=ecl)
      ######
      #step 2: update fixed effect etahat with mle
      ...
      dsend <- xtil[, -c(1:np)]
      zsend <- xtil[, 1:np]
      cvec <- family$linkfun(muhat) + (ysim - muhat) / family$variance(muhat)
      zvec <- uinv %*% cvec
      if (nrep > 1) {
        ans <- suppressMessages(try(coneB(zvec, dsend, vmat = zsend, face = face), silent = TRUE))
      } else {
      ...
      #}
      #etahat <- dd %*% bhat
      etahat <- bigmat_t %*% bhat
      mufh <- muhat.fun(etahat=etahat, fml='binomial')
      ecl <- f_ecl(etahat, ncl, szs)
      muhat <- c(unlist(mapply(function(e1, e2) family$linkinv(e1+e2), e1=ecl, e2=avec))) #include fixed and random effect
      diff <- mean((oldmu - muhat)^2)
      mdiff <- abs(max(muhat) - 1) > 1e-6
      #print (bhat)
    }
...
cgamm.pv = function(y, xmat, zmat, shapes, delta=NULL, np=NULL, capms=NULL,
                    numknotsuse=NULL, varlist=NULL, family=gaussian(), weights=NULL,
                    ycl=NULL, siga=NULL, ahat=NULL, ncl=NULL, szs=NULL,
                    test_id=1, nsims=100, nAGQ=1L, uinv_vm=NULL) {
  n = length(y)
  cicfamily = CicFamily(family)
  llh.fun = cicfamily$llh.fun
  #new: use log link in gamma
...
cgamm.pvz = function(y, bigmat, df_obs, sse1 = NULL, np = 1, zid = 1, zid1 = 1, zid2 = 1,
                     ...
                     ycl=NULL, siga=NULL, ahat=NULL, ncl=NULL, szs=NULL, nAGQ=1L) {
  n = length(y)
  cicfamily = CicFamily(family)
  llh.fun = cicfamily$llh.fun
  #new: use log link in gamma
...
###############
predict.cgamm = function(object, newdata, interval = c("none", "confidence", "prediction"), type = c("response", "link"), level = 0.95, n.mix = 100, var.f = NULL,...) {
  ...
  for (i in 1:nc) {
    ...
    if (shpi >= 9) {
      ...
      splpri = dpri$amat
      if (shpi > 10 & shpi < 13) {
        #new: 2026; in the makedelta from dropbox cgam, ms it the transpose of the old ms
    ...
    if (family$family == "gaussian") {
      ...
      if (is.null(var.f)) {
        ...
        extras <- list(muhat=muhat, n=n, np=np, m_acc=m_acc, szs=szs, balanced=balanced, 
                       uinv0=uinv0, thhat=thhat, prior.w=prior.w, siga2hat=siga2hat, sig2hat=sig2hat, edges=edges, wt.iter=wt.iter)
        if(multicore){
          tot_cores <- parallel::detectCores()
          common_args <- list(X = 1:nloop, FUN = .compute_mixture_cgamm, mc.cores = (tot_cores - 2))
          mixture_rslt <- do.call(parallel::mclapply, c(common_args, extras))
          #mixture_rslt <- parallel::mclapply(1:nloop, .compute_mixture_cgamm, muhat = muhat,
          #                                   n = n,  np = np, m_acc = m_acc, szs = szs, balanced = balanced, 
          #                                   uinv0 = uinv0, thhat = thhat, prior.w = prior.w, 
          #                                   siga2hat = siga2hat, 
          #                                   sig2hat = sig2hat, edges = edges, mc.cores = (tot_cores-2))
        }else{
          common_args <- list(X = 1:nloop, FUN = .compute_mixture_cgamm)
          mixture_rslt <- do.call(lapply, c(common_args, extras))
          #mixture_rslt <- lapply(1:nloop, .compute_mixture_cgamm, muhat = muhat,
          #                       n = n,  np = np, m_acc = m_acc, szs = szs, balanced = balanced, 
          #                       uinv0 = uinv0, thhat = thhat, prior.w = prior.w, 
          #                       siga2hat = siga2hat, 
          #                       sig2hat = sig2hat, edges = edges)
        }
        ...
        # Get names of all columns EXCEPT freq and prob
        other_cols <- colnames(bsec)[!colnames(bsec) %in% c('freq', 'prob')]
        # Reorder: freq and prob first, then everything else
        bsec <- bsec[, c("freq", "prob", other_cols)]
        
        #bsec_tmp <- bsec
        #bsec_tmp[, 1] <- bsec$freq
        #bsec_tmp[, 2] <- bsec$prob
        #bsec_tmp[, -c(1,2)] <- bsec[, !colnames(bsec) %in% c('freq', 'prob')]
        #bsec <- bsec_tmp
      ...
      if ("confidence" %in% interval) {
        ...
        hl = mult*sqrt(var.f)
        #new: 1/7/26 make coverage rate checking easier
        #test more
        lwr_etas <- upp_etas <- NULL
        varlst <- object$varlist
        p <- length(unique(varlst)) #only smooth terms
        nz <- object$d0
        cov_beta <- acov
        bhat <- object$coefs
        if(nz > 0){
          cov_beta <- acov[-c(1:nz), -c(1:nz)]
          bhat <- bhat[-c(1:nz)]
        }
        #only show smooth terms?
        dd_s <- t(splpr)
        for(k in 1:p){
          ps_k <- which(varlst == k)
          dd_k <- dd_s[, ps_k, drop = FALSE]
          eta_k <- dd_k %*% bhat[ps_k, drop = FALSE]
          cov_k <- cov_beta[ps_k, ps_k]
          sd_eta_k <- diag(dd_k %*% cov_k %*% t(dd_k))^.5
          lwr_eta_k <- eta_k - mult * sd_eta_k
          upp_eta_k <- eta_k + mult * sd_eta_k
          lwr_etas <- rbind(lwr_etas, c(lwr_eta_k))
          upp_etas <- rbind(upp_etas, c(upp_eta_k))
        }
      }
      if ("prediction" %in% interval) {
        #imat = diag(n)
      ...
      if ("confidence" %in% interval) {
        ans = list(fit = muhatpr, lower = muhatpr - hl, upper = muhatpr + hl, 
                   var.f=var.f, acov = acov, lwr_etas = lwr_etas, upp_etas = upp_etas)
      }
      if ("prediction" %in% interval) {
      ...
      shat = siga2hat / thhat
      vm = diag(1/w) + c(siga2hat)*oneMat
      #vm = (diag(1/w) + c(siga2hat)*oneMat)/shat #old, before 2025
      #new: 2025 Nov
      shat = 1
      umat_vm = t(chol(vm))
      uinv_vm = solve(umat_vm)
      ...
      multicore <- getOption("cgam.parallel")
      wt.iter <- TRUE
      extras <- list(thhat_mu=thhat_mu, n=n, m=m, uinv=uinv, deltil=deltil, 
                     atil=atil, imat=imat, shat=shat, wt.iter=wt.iter)
      if(multicore){
        tot_cores <- parallel::detectCores() - 2
        common_args <- list(X = 1:nloop, FUN = .compute_mixture_cgamm, mc.cores = (tot_cores - 2))
        cmat_lst <- do.call(parallel::mclapply, c(common_args, extras))
        #cmat_lst <- parallel::mclapply(1:nloop, .compute_vcov_cgamm,
        #                               thhat_mu=thhat_mu, n=n, m=m, 
        #                               uinv=uinv, deltil=deltil, atil=atil, 
        #                               imat=imat, shat=shat, mc.cores=tot_cores)
      } else {
        common_args <- list(X = 1:nloop, FUN = .compute_mixture_cgamm)
        cmat_lst <- do.call(lapply, c(common_args, extras))
        #cmat_lst <- lapply(1:nloop, .compute_vcov_cgamm,  thhat_mu=thhat_mu, n=n, m=m, 
        #                   uinv=uinv, deltil=deltil, atil=atil, 
        #                   imat=imat, shat=shat, wt.iter=wt.iter)
      }
      ...
      if ("confidence" %in% interval) {
        if ("response" %in% type) {
          ...
          ans = list(fit = etapr, lower = loweta, upper = uppeta, se.fit = se.fit,  
                     acov = cmat, lwr_etas = lwr_etas, upp_etas = upp_etas)
        }
      }
      if ("prediction" %in% interval) {
...
#--------------------------------------------------------
#new: 2026 parallel the covariance matrix computation
#--------------------------------------------------------
.compute_mixture_cgamm <- function(isim,...)
{
  args <- list(...)
  wt.iter <- args$wt.iter
  #print (names(args))
  if(!wt.iter){
    muhat <- args$muhat
    n <- args$n
    ...
    uinv0 <- args$uinv0
    thhat <- args$thhat
    prior.w <- args$prior.w
    siga2hat <- args$siga2hat
    sig2hat <- args$sig2hat
    edges <- args$edges
...
summary.cgamm <- function(object,...) {
  if (!is.null(object$zcoefs)) {
    ...
    #	ans
    #} else {
    ans <- list(call = object$call, coefficients = rslt1, coefficients2 = rslt2, zcoefs = coefs, cic = cic, resid_df_obs = resid_df_obs, family = family)
...
plot.cgamm <- function(x,...)
{
  ...
  p <- ncol(xmat_add)
  xmat <- t(object$bigmat)
  nz <- object$d0 #nz inclused intercept, nog like cdc
  bhat <- object$coefs
  if(nz > 0){
    xmat <- xmat[, -c(1:nz)] #spl #remove z and 1 only
    bhat <- bhat[-c(1:nz)]
  }
  varlst <- object$varlist #named as varlist in cgamm, tracks smooth terms
  ...
  #test
  if(!exists("cov_beta", where = extras)){
    cov_beta <- NULL
  } else {
    cov_beta <- extras$cov_beta
  }
  ...
  if(!exists("true_etas", where = extras)){
    true_etas <- NULL
  } else {
    true_etas <- extras$true_etas
  }
  if(!exists("use_ggplot", where = extras)){
    use_ggplot <- FALSE
  } else {
    use_ggplot <- extras$use_ggplot
  }
  if(!exists("dynamic", where = extras)){
    dynamic <- FALSE
  } else {
  ...
  if(!use_ggplot & !dynamic){
    ...
    if(pages == 1){
      oldpar <- par(mfrow = layout_for_cgam(n_plots = p, pages = pages))
    } else {oldpar <- par(mfrow = c(1, p))}
  }
  if(!use_ggplot & dynamic){
    oldpar <- par(mfrow = c(1, 1))
  }
  pic_lst <- vector("list", length = p)
  for (k in 1:p) {
    #change the following to be %*% when using splines
    #eta_k <- xmat_alt[obs, k] * beta[k]
    ps_k <- which(varlst == k)
    dd_k <- xmat[, ps_k, drop = FALSE]
    eta_k <- dd_k %*% bhat[ps_k, drop = FALSE]
    if(!is.null(cov_beta)){
      cov_k <- cov_beta[ps_k, ps_k]
      sd_eta_k <- diag(dd_k %*% cov_k %*% t(dd_k))^.5
    ...
    if(use_ggplot){
      ...
      labs <- ggplot2::labs
      df <- data.frame(x = x_k, eta = eta_k, lwr = if (!is.null(cov_beta)) lwr_eta_k else NA, 
                       upp = if (!is.null(cov_beta)) upp_eta_k else NA)
      pic <- ggplot(df, aes(x = x)) +
        geom_line(aes(y = eta), linewidth = 0.5) +
        labs(
          x = bquote(x[.(k)]),
          y = bquote(hat(eta)[.(k)] ~ "(black solid)"),
          title = bquote(~ hat(eta)[.(k)] ~ "vs" ~ x[.(k)])
        ) +
        ylim(ylim_use)
      
      if (!is.null(cov_beta)) {
        pic <- pic +
          geom_line(aes(y = lwr), color = "red", linewidth = 0.3) +
      ...
      if(dynamic){
        ...
        pic_lst[[k]] <- pic + theme_use
      }
    } else {
      plot(x_k[ord_k], eta_k[ord_k], xlab = bquote(x[.(k)]), ylab = bquote(hat(eta)[.(k)]),
           ylim = ylim_use, 
           main = bquote(~ hat(eta)[.(k)] ~ "vs" ~ x[.(k)]),
           type = "l", lwd = 1)
      
      if(!is.null(cov_beta)){
        lines(x_k[ord_k], lwr_eta_k[ord_k], col=2, cex=.2)
        lines(x_k[ord_k], upp_eta_k[ord_k], col=2, cex=.2)
      }
      
      if(!is.null(true_etas)){
        lines(x_k[ord_k], true_etas[ord_k, k], col=3, cex=.2)
      }
